...
| `notification:sent:{eventId}:{memberId}` | String | 24시간 | 중복 발송 방지 (보조) |
...
### D-6. 멱등성 보장 — DB가 최종 보장, Redis는 보조
`notifications` 테이블에 `UNIQUE(event_id, member_id)` 제약을 두어 DB 레벨에서 중복 인박스 생성을 원천 차단. Redis `notification:sent:{eventId}:{memberId}` 키(TTL 24h)는 DB 조회 없이 빠른 중복 체크를 위한 보조 수단. 서버 장애로 ACK 전에 프로세스가 죽어 Redis PEL에서 재처리되더라도 DB UNIQUE 제약이 최종 방어선 역할을 한다.
